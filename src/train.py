...
def run(df, fold, model, error):
    ...
    if config.SAVE_MODELS:
        ...
        joblib.dump(clf, os.path.join(config.MODEL_OUTPUT, f"{model}_{fold}_{config.TRAIN_VERSION}.bin"))
    return clf
...
if __name__ == "__main__":
    ...
    df = pd.read_csv(join(config.DATA_PATH, "train_{}_processed.csv".format(config.TRAIN_VERSION)))
